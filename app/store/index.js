...
export const TAB_GROUP_VIEWS = ['default', 'group-by-date', 'group-by-month']
...
const store = new Vuex.Store({
    ...
    actions: {
        async SAVE_TAB_GROUP({ commit, dispatch, state }, { filter, window }) {
    ...
    getters: {
        sortedAndFilteredTabGroups: state => {
            let groups = state.data.tabGroups.filter(a => !(a.title && a.url))

            if (state.tabGroupView === 'group-by-date') {
                groups = groupByDate(groups)
            } else if (state.tabGroupView === 'group-by-month') {
                groups = groupByDate(groups, 'MMYYYY')
            } else {
